<?php

namespace User\V1\Notification\Email\Listener;

use User\V1\UserActivationEvent;

class ActivationEventListener extends Listener
{
    /**
     * @param  \Laminas\EventManager\EventManagerInterface  $events
     * @param  int  $priority
     * @return void
     */
    public function attach($events, $priority = 1)
    {
        $this->listeners[] = $events->attach(
            UserActivationEvent::EVENT_ACTIVATE_USER_SUCCESS,
            [$this, 'sendActivationEmail'],
            499
        );
    }

    /**
     * Run Console to Send Activation Email
     *
     * @param  \User\V1\UserActivationEvent  $event
     * @return int
     */
    public function sendActivationEmail($event)
    {
        $emailAddress = $event->getUserActivationEntity()->getUser()->getUsername();
        // command: laminas-cli user:send-activation-email <emailAddress>
        $cli = $this->processBuilder
            ->setArguments(['user:send-activation-email', $emailAddress])
            ->getProcess();
        $cli->start();
        $pid = $cli->getPid();

        $this->logger->log(
            \Psr\Log\LogLevel::DEBUG,
            "{function} {pid} {commandline}",
            [
                "function" => __FUNCTION__,
                "commandline" => $cli->getCommandLine(),
                "pid" => $pid
            ]
        );
    }
}
